---import React, { useState } from 'react'; import { Text, TextInput, Button, View, FlatList, TouchableOpacity, Alert } from 'react-native'; export default function App() { const [code, setCode] = useState(''); const [nickname, setNickname] = useState(''); const [isAuthenticated, setIsAuthenticated] = useState(false); const [messages, setMessages] = useState([]); const [newMessage, setNewMessage] = useState(''); const [groupCode, setGroupCode] = useState('ABC123'); // Groepscode die gedeeld wordt // Handelt het inloggen met groepscode en bijnaam const handleJoinGroup = () => { if (code === groupCode && nickname.trim() !== '') { setIsAuthenticated(true); } else { Alert.alert("Fout", "De groepscode is onjuist of je hebt geen bijnaam ingevoerd!"); } }; // Bericht versturen naar de groep const sendMessage = () => { if (newMessage.trim()) { setMessages([...messages, { id: Math.random().toString(), text: newMessage, nickname }]); setNewMessage(''); } }; // Verwijder account en berichten const deleteAccount = () => { Alert.alert( "Account verwijderen", "Weet je zeker dat je je account en alle gegevens wilt verwijderen?", [ { text: "Cancel", style: "cancel" }, { text: "OK", onPress: () => { setMessages([]); setNickname(''); }} ] ); }; return ( <View style={{ padding: 20 }}> {!isAuthenticated ? ( <View> <TextInput value={nickname} onChangeText={setNickname} placeholder="Kies een bijnaam" style={{ borderWidth: 1, marginBottom: 10, padding: 10 }} /> <TextInput value={code} onChangeText={setCode} placeholder="Voer groepscode in" style={{ borderWidth: 1, marginBottom: 10, padding: 10 }} /> <Button title="Doe mee met groep" onPress={handleJoinGroup} /> </View> ) : ( <View> <FlatList data={messages} renderItem={({ item }) => ( <View style={{ padding: 5, backgroundColor: '#f0f0f0', marginBottom: 5 }}> <Text><strong>{item.nickname}:</strong> {item.text}</Text> </View> )} keyExtractor={(item) => item.id} /> <TextInput value={newMessage} onChangeText={setNewMessage} placeholder="Typ een bericht" style={{ borderWidth: 1, marginBottom: 10, padding: 10 }} /> <Button title="Stuur bericht" onPress={sendMessage} /> <TouchableOpacity onPress={deleteAccount}> <Text style={{ color: 'red', marginTop: 10 }}>Verwijder account en berichten</Text> </TouchableOpacity> </View> )} </View> ); }
id: build-speed
